...
- [x] **T1.4** Add capture command handlers to the pipeline worker (`src/engine/worker.ts`): `capture-transfer-streams` (carries the streams plus track settings; capture starts on receipt — a separate `capture-start` command was dropped because capture must begin from a main-thread user gesture) and `capture-stop` (encoder flush + final buffer state). Pause/resume is not in v1.
- [x] **T1.5** Add `capture-session-state` state message from worker to UI: streams session info (active, elapsed, track presence) at low frequency (every 500 ms).
