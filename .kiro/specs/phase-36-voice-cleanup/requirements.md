...
- **R1.4** A/B bypass: the worklet provides a per-track bypass toggle that
  ...
  SAB layout's reserved denoiser slot (`SAB[34]` = per-track bypass bitmap,
  packed into a `Float32`-encoded bitmask, one bit per track index up to 32
  tracks). Track count beyond 32 uses a second SAB word at `SAB[35]`; the
  denoiser bypass bitmask occupies `SAB[34..35]` (2 words, 64 tracks max).
...
- **R2.2** The analysis pass reads the master mix by calling `mixAudioWindow`
  (from `src/engine/export.ts`) for successive 400 ms windows, advancing the
  window by 100 ms each step, until the full timeline duration is consumed.
  No whole-file buffer is held in memory; at most two overlapping windows are
  live at once.
