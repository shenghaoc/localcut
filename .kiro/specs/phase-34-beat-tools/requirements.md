...
- **R1.2** Analysis reads audio through `SequentialAudioSource.pcmWindowAt`
  ...
  `frameCount = 48_000 × 10` (10 s); at most 3 windows overlap in flight
...
- **R1.4** The concrete DSP pipeline (all parameters are fixed, not
  ...
  - Onset peak-picking: moving-mean threshold with a **state window of 16
    ...
    `max(threshold × moving_mean, 0.01)`.
  ...
  - Beat grid: the best-tempo period `T` is phase-aligned to the onset
    envelope by sliding a phase `φ ∈ [0, T)` in steps of `T / 128` and
    choosing the `φ` that maximises `Σ onset_strength(φ + n × T)` over all
    onset frames; the beat grid is `{ φ + n × T | n ≥ 0, φ + n × T ≤ duration }`.
...
- **R5.3** **Align mode:** each selected clip's `start` is moved to the
  ...
  beats, snap to the earlier beat. The move is clamped to `[0, ∞)`. No clip is
