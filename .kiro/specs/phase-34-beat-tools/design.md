...
by exhaustive scan over `T / 128` steps costs O(128 × N_onsets) per tempo
...
- Hann window: `w[n] = 0.5 × (1 − cos(2π n / (N − 1)))`, N = 1024.
...
```
flux[t] = Σ_k  max(0,  log(1 + |X_t[k]|) − log(1 + |X_{t−1}[k]|))
```
...
Parameters: state window W = 16 frames, multiplier α = 1.3, min gap G = 0.25 s
(= ceil(0.25 × 48000 / 512) = 24 frames).

```
moving_mean[t] = mean(flux[t−W … t])
threshold[t]   = max(α × moving_mean[t], 0.01)
...
```
lag_samples(bpm) = round(60 × 48000 / (512 × bpm))
```

Lags span approximately [14, 47] frames. For each candidate lag `l`:

```
acf[l] = Σ_{t=0}^{T−l−1} flux[t] × flux[t+l]
```
...
```
peak_l = argmax(acf)
delta   = 0.5 × (acf[peak_l−1] − acf[peak_l+1])
              / (acf[peak_l−1] − 2×acf[peak_l] + acf[peak_l+1])
refined_lag = peak_l + delta
tempoBpm    = 60 × 48000 / (512 × refined_lag)
```

### Stage 6 — Beat-grid phase alignment

Period `T_frames = 60 × 48000 / (512 × tempoBpm)` frames.
Scan `φ ∈ [0, T_frames)` in steps of `T_frames / 128`:

```
score(φ) = Σ_n  flux[round(φ + n × T_frames)]  for all valid n
```

Choose `φ*` = argmax score. Beat times in seconds:

```
beat_time_s[n] = (φ* + n × T_frames) × 512 / 48000
```
