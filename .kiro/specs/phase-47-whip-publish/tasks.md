...
- [ ] **T2.1** `src/engine/whip-session.ts`: main-thread `WhipSession` with an
...
- [ ] **T2.2** Typed `PublishState` machine (`idle` / `connecting` / `live` /
...
- [ ] **T4.3** Gate simultaneous record+stream on a second lease being
  ...
  recording or export.
- [ ] **T4.4** Missing `RTCPeerConnection` (or other required probe) hides the
  publish feature behind a reduced-tier explanation — shell stays alive.

## T5 — Program-feed tap (R4)

- [ ] **T5.1** `src/engine/publish-frame-tap.ts` (worker): clone the
  ...
  closed and counted.
- [ ] **T5.2** Close-exactly-once across write/drop/stop/error paths,
  including pending clones at stop — Phase 27 discipline.
- [ ] **T5.3** Worker-side generator + transferable track as the primary mode;
...
- [ ] **T5.4** Audio: master-bus tap via `MediaStreamAudioDestinationNode` on
  the existing `AudioContext` so the stream matches the program monitor
  (post-gain/pan/fades).
- [ ] **T5.5** Protocol messages in `src/protocol.ts`: `publish-tap-start` /
...
- [ ] **T6.1** `src/ui/PublishPanel.tsx`: endpoint-type presets (Twitch WHIP,
...
- [ ] **T6.2** RTMP honesty copy: YouTube/Douyin/Bilibili need a user-supplied
...
- [ ] **T6.3** Live state display: connection phase, achieved vs target
  bitrate, reconnect attempt counter; failure reasons use the T1.3 mapping,
  not raw exceptions.
- [ ] **T6.4** UI-standards + accessibility pass: keyboard reachable, ARIA
...
- [ ] **T7.1** `PUBLISH_SETTINGS_STORE` in `src/engine/persistence.ts`:
  app-scoped, outside `ProjectDoc` — no project schema bump.
- [ ] **T7.2** Token is session-only by default; "remember token on this
  device" opt-in with plain unencrypted-storage copy.
- [ ] **T7.3** Test proving Phase 23 bundle serialization and `ProjectDoc`
...
- [ ] **T9.3** `encoder-budget.test.ts`: acquire/release, exhaustion blocks
  before connection creation, double-release guard, record+stream gating.
- [ ] **T9.4** `publish-frame-tap.test.ts`: mocked generator writer —
  latest-frame-wins, drop counting, close-exactly-once across stop/error.
- [ ] **T9.5** Protocol type guards for the new commands/messages; all tests
  Node-environment, co-located, no media fixtures; test count grows.

## T10 — CI integration: MediaMTX + Playwright (R8)

- [ ] **T10.1** Add `@playwright/test` (devDependency, npm only) and a single
  publish-flow spec; everything else stays in Vitest.
- [ ] **T10.2** New CI job (separate from build/test) running a
  `bluenviron/mediamtx` container; Chromium publishes a synthetic program
  feed to `http://localhost:8889/<path>/whip`.
- [ ] **T10.3** Assert ingest via the MediaMTX API: session present and
  `bytesReceived` increasing.
- [ ] **T10.4** Assert teardown: user stop issues `DELETE` and the MediaMTX
  session disappears.
- [ ] **T10.5** Assert reconnect: restart the container mid-stream; client
  walks `reconnecting` → `live` with a fresh session per the documented
  policy.

## T11 — Docs + verification (R8)

- [ ] **T11.1** `docs/LIVE-STREAMING.md`: per-endpoint setup, WHIP→RTMP
...
- [ ] **T11.3** `npm run build` and `npm test` green; test count grows.
