...
- [x] **T1.5** Probe both `requestAdapter()` (standard) and `requestAdapter({ featureLevel: 'compatibility' })` in sequence; set `compatibilityAdapter: true` only when the standard probe fails but the compat probe succeeds.
