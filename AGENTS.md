...
- **Quality gate in CI-like runs** — `npm run lint` (ESLint), `npm run format:check` (Prettier), `npm run build` (strict `tsc` + Vite), and `npm test` (Vitest, Node environment). All four run automatically via `.github/workflows/ci.yml`.
