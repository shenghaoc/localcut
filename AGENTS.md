...
- **Quality gate in CI-like runs** — `vp install` to install, then `vp run check` (format:check + lint + typecheck + Vitest + production build), mirroring the `quality_gate` job in `.circleci/config.yml`.
