...
vp run typecheck:tsc # tsc --noEmit compatibility check
```
...
- Use `vp run typecheck:tsc` only as a compatibility check or when investigating a `tsgo` native-preview discrepancy.
...
1. `vp run check` → green (format:check + lint + `tsgo --noEmit` + Vitest + production build). This is what CI runs after `vp install`. Keep `vp run typecheck:tsc` available for TypeScript 6 compatibility comparisons.
