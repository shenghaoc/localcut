...
- [**Phase 46: Replay Buffer + Live Audio Chain**](.kiro/specs/phase-46-replay-buffer-live-audio/tasks.md) — always-on GOP-aligned replay ring buffer with OPFS spill; instant clip drop to timeline; live audio gate/compressor/limiter/denoiser chain in the monitor path.
...
- [**Bugfix: Capability-probe false negatives + editor chrome overlap & IA**](.kiro/specs/bugfix-capability-probe-and-editor-overlap/tasks.md) — one spec, two parts. **Part 1 (B1–B9, Implemented — merged in #130):** features falsely reported unavailable on a fully capable Chromium profile (WebGPU + WebNN + COOP/COEP). H.264 probe codec strings encode Level 3.0, too low for their probe resolutions, so `VideoEncoder.isConfigSupported` rejects them → H.264/MP4 export silently dropped and realtime-encode falsely "unavailable"; the OPFS `createSyncAccessHandle` probe runs on the main thread where the API doesn't exist (it's Worker-only) → recording/Program Mode blocked; recording hard-requires transferable MediaStreamTrack (honest gate + flag workaround shipped; the off-main-thread fallback is a separate branch). Plus a CSS regression where PR #125's duplicate `.workspace` rules override the responsive collapse (left dock/right side-rail overlap), two misleading strings (WebNN never surfaced; Live Audio Chain "Noise Suppression" placeholder), and a media-bin delete button pushed out of frame by a hidden horizontal scrollbar. **Part 2 (B10–B16) — editor-chrome navigation/IA reorganization (this branch)** from the [panels audit](audits/editor-chrome-panels-2026-06-20/audit.md) — three competing nav systems expose the same concepts through different labels (right rail crams 7 tabs ≈374px into ~302px behind a hidden scrollbar; left rail looks like tabs but fires mixed actions / `Media` is a dead label; menus duplicate the toolbar; bare `Cleanup` means two surfaces). Fix: consolidate by user job — right rail → ≤4 contextual destinations with secondary segmented controls and no scrollable primary nav; left rail → honest library switcher; menu = taxonomy, toolbar = frequent actions; one label per concept; compact unavailable-state disclosures. **Phase 1 (IA-T1–IA-T3: labels/dedupe/density) Implemented.** Phases 2–3 (nav restructure) remain proposed but are not part of this release.
