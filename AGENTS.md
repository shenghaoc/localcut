...
- [**Editor chrome information architecture**](.kiro/specs/feature-editor-chrome-ia/tasks.md) — IA/navigation reorganization from the [editor-chrome panels audit](audits/editor-chrome-panels-2026-06-20/audit.md). Three competing navigation systems expose the same concepts through different labels: the **right rail** crams seven text tabs (Inspector/Captions/Record/Program/Replay/Audio/Cleanup ≈ 374px) into a ~302px panel behind a hidden horizontal scrollbar, so tabs clip and activating `Cleanup` scrolls `Inspector` off-edge; the **left rail** looks like persistent tabs but fires mixed actions (import picker, right-rail switches, floating overlays, modals; `Media` is a dead label); the **top menu/toolbar** duplicate access (`Browser capabilities`, `Help` in two places) and repeat `Search actions…` in every menu; and bare `Cleanup` means *Local Audio Cleanup* (toolbar) vs *Voice Cleanup* (right rail). Fix: consolidate by user job — right rail → ≤4 contextual destinations (`Inspector`/`Text`/`Audio`/`Capture`) with secondary segmented controls and no scrollable primary nav; left rail → honest library switcher; menu bar = taxonomy, toolbar = frequent actions; one label per concept; compact unavailable-state disclosures. Phased rollout (labels/dedupe → right rail → left rail/Beats).

- [**Bugfix: Capability-probe false negatives + editor chrome overlap**](.kiro/specs/bugfix-capability-probe-and-editor-overlap/tasks.md) — features falsely reported unavailable on a fully capable Chromium profile (WebGPU + WebNN + COOP/COEP). H.264 probe codec strings encode Level 3.0, too low for their probe resolutions, so `VideoEncoder.isConfigSupported` rejects them → H.264/MP4 export silently dropped and realtime-encode falsely "unavailable"; the OPFS `createSyncAccessHandle` probe runs on the main thread where the API doesn't exist (it's Worker-only) → recording/Program Mode blocked; recording hard-requires transferable MediaStreamTrack while publish degrades via a main-frames fallback. Plus a CSS regression where PR #125's duplicate `.workspace` rules override the responsive collapse (left dock/right side-rail overlap below ~1236px), and two misleading strings (WebNN never surfaced as a capability; Live Audio Chain "Noise Suppression — available in a future update" despite shipped Voice/Local Audio Cleanup).
