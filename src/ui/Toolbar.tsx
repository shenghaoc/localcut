...
export function Toolbar(props: ToolbarProps) {
  ...
  const timelineModeValues = createMemo(() => {
    ...
    if (props.timelineSnapEnabled && props.timelineSnapToBeats)
      values.push("beat");
    return values;
  });
  ...
  const sourceFormatLabel = () => {
    ...
    const fps = video.frameRate
      ? `${Math.round(video.frameRate)} FPS`
      : "FPS ?";
    ...
    <header class="toolbar">
      ...
      <div class="toolbar-main">
        ...
        <div class="toolbar-center">
          <span
            class="file-name"
            title={
              props.metadata?.fileName ?? "Drop or import a file to get started"
            }
          >
            <Show
              when={props.metadata}
              fallback="Drop or import a file to get started"
            >
              {(meta) => meta().fileName}
            </Show>
          </span>
        ...
        <div class="toolbar-right">
          ...
          <div class="toolbar-timecode" aria-label="Playback timecode">
            <span>
              {formatToolbarTimecode(
                props.currentTime(),
                props.metadata?.video?.frameRate ?? null,
              )}
            </span>
            <small>/</small>
          ...
          <div class="master-mix" role="group" aria-label="Master mix">
            ...
            <label class="master-fader">
              ...
              <input
                ...
                aria-valuetext={
                  Number.isFinite(props.masterGain)
                    ? props.masterGain.toFixed(2)
                    : "0.00"
                }
              />
      ...
      <div class="pipeline-strip" aria-label="Pipeline status">
        ...
        <span
          class={cn(
            "pipeline-chip",
            props.crossOriginIsolated ? "is-ok" : "is-warn",
          )}
        >
        ...
        <button
          type="button"
          class={cn(
            "pipeline-chip pipeline-chip-button is-tool",
            props.publishLive && "is-live",
          )}
