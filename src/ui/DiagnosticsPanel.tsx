...
export function DiagnosticsPanel(props: DiagnosticsPanelProps) {
  ...
  const reportText = createMemo(() => {
    ...
    if (!snapshot) return "";
    return formatCopyableDiagnosticReport(
      buildCopyableDiagnosticReport(snapshot, props.sources),
    );
  });
  ...
  async function copyReport() {
    ...
    setCopyStatus(
      result.ok ? "Diagnostics report copied." : `Copy failed: ${result.error}`,
    );
  }
    ...
    <Show when={props.open}>
      ...
      >
        <header class="capability-panel-header">
          <div>
            ...
            <p class="capability-panel-tier">
              <Show
                when={props.snapshot}
                fallback="Couldn't capture a snapshot"
              >
        ...
        <Show
          when={props.snapshot}
          fallback={
            <p class="capability-panel-note">Requesting diagnostics...</p>
          }
        >
          {(snapshot) => (
            <>
              ...
              <section class="diagnostics-section">
                ...
                <dl class="diagnostics-grid">
                  ...
                  <div>
                    <dt>Features</dt>
                    <dd>
                      {snapshot().capability.webGpu.features.join(", ") ||
                        "default"}
                    </dd>
                  </div>
                    ...
                    <dd>
                      {snapshot()
                        .capability.webCodecs.decoders.map(
                          (c) =>
                            `${c.codec}${c.supported ? "" : " (unsupported)"}`,
                        )
                        .join(", ") || "none"}
                    </dd>
                  </div>
                    ...
                    {(compat) => (
                      <>
                        <div>
                          <dt>Video codecs</dt>
                          <dd>
                            {compat().supportedVideoCodecs}/
                            {compat().totalVideoCodecs} supported (
                            {compat().hwPreferredVideoCodecs} hardware) ·{" "}
                            {compat()
                              .videoCodecs.map(
                                (c) => `${c.codec}: ${c.strategy}`,
                              )
                              .join(", ") || "none"}
                          </dd>
                        </div>
                        <div>
                          <dt>Audio codecs</dt>
                          <dd>
                            {compat().supportedAudioCodecs}/
                            {compat().totalAudioCodecs} supported ·{" "}
                            {compat()
                              .audioCodecs.map(
                                (c) => `${c.codec}: ${c.strategy}`,
                              )
                        ...
                        <div>
                          <dt>Containers</dt>
                          <dd>
                            {compat().demuxableContainers.join(", ") || "none"}
                          </dd>
                        </div>
                      </>
                    )}
              ...
              <section class="diagnostics-section">
                ...
                <dl class="diagnostics-grid">
                  ...
                  <div>
                    <dt>Checksum</dt>
                    <dd>
                      {snapshot().voiceCleanup.wasmSha256 ?? "not verified yet"}
                    </dd>
                  </div>
                  <div>
                    <dt>Latency</dt>
                    <dd>
                      {snapshot().voiceCleanup.workletLatencyMs.toFixed(2)} ms
                    </dd>
                  </div>
                  <div>
                    <dt>Target</dt>
                    <dd>
                      {snapshot().voiceCleanup.normalisationTargetLufs} LUFS
                    </dd>
                  </div>
                  <div>
                    <dt>Ceiling</dt>
              ...
              <section class="diagnostics-section">
                ...
                <Show
                  ...
                  fallback={<p>All diagnostics passed — no issues detected.</p>}
                >
                  <ul class="diagnostics-list">
                    <For each={snapshot().recoveryActions}>
                      {(action) => (
                        <li
                          class={`diagnostics-row ${action.enabled ? "is-warn" : "is-muted"}`}
                        >
                          ...
                          <Show
                            ...
                            fallback={
                              <Show when={action.reasonDisabled}>
                                <p class="diagnostics-disabled-reason">
                          ...
                          >
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                props.onRecoveryAction?.(action.actionId)
                              }
                            >
                              {action.label}
                            </Button>
