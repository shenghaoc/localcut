    ...
    <Show when={props.open}>
      ...
      >
        <header class="capability-panel-header">
          <div>
            <p class="panel-title" id="diagnostics-panel-title">Diagnostics</p>
          ...
          <div class="diagnostics-actions">
            <Button size="icon" variant="ghost" onClick={props.onRefresh} aria-label="Refresh diagnostics">
              <RefreshCw size={16} aria-hidden="true" />
            </Button>
            <Button size="icon" variant="ghost" onClick={copyReport} aria-label="Copy diagnostics report">
              <Clipboard size={16} aria-hidden="true" />
            </Button>
            <Button size="icon" variant="ghost" onClick={props.onClose} aria-label="Close diagnostics panel">
              <X size={16} aria-hidden="true" />
            </Button>
          </div>
        </header>

        <Show when={props.snapshot} fallback={<p class="capability-panel-note">Requesting diagnostics...</p>}>
          {(snapshot) => (
            <>
              <section class="diagnostics-section">
                ...
                <ul class="diagnostics-list">
                  <For each={snapshot().capability.findings}>
                    {(finding) => (
                      <li class={`diagnostics-row ${finding.status === 'supported' ? 'is-ok' : 'is-warn'}`}>
              ...
              <section class="diagnostics-section">
                <h2>Storage + Cache</h2>
                <dl class="diagnostics-grid">
                  <div><dt>Usage</dt><dd>{formatBytes(snapshot().storage.usageBytes)}</dd></div>
                  <div><dt>Quota</dt><dd>{formatBytes(snapshot().storage.quotaBytes)}</dd></div>
                  <div><dt>OPFS</dt><dd>{snapshot().storage.opfsSupported ? 'available' : 'unavailable'}</dd></div>
                  <div><dt>Cache</dt><dd>{snapshot().proxyCache.status} · {formatBytes(snapshot().proxyCache.estimatedBytes)}</dd></div>
                </dl>
              </section>

              <section class="diagnostics-section">
                <h2>Export</h2>
                <Show when={snapshot().activeExportSettings} fallback={<p>No active export settings.</p>}>
                  {(settings) => (
                    <dl class="diagnostics-grid">
                      <div><dt>Codec</dt><dd>{settings().codec.toUpperCase()}</dd></div>
                      <div><dt>Container</dt><dd>{settings().container.toUpperCase()}</dd></div>
                      <div><dt>Size</dt><dd>{settings().width}x{settings().height}</dd></div>
                      <div><dt>Source</dt><dd>{settings().sourceMode}</dd></div>
                    </dl>
                  )}
                </Show>
              </section>
              ...
              <section class="diagnostics-section">
                <h2>Recent Errors</h2>
                <Show when={snapshot().recentErrors.entries.length > 0} fallback={<p>No recent structured errors.</p>}>
                  <ul class="diagnostics-list">
              ...
              <section class="diagnostics-section">
                ...
                <Show when={snapshot().recoveryActions.length > 0} fallback={<p>No recovery actions are available.</p>}>
                  <ul class="diagnostics-list">
        ...
        <p class="diagnostics-copy-status" aria-live="polite">{copyStatus()}</p>
      </aside>
    </Show>
  );
}
