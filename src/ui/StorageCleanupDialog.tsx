...
function pressureClass(pressure: StorageHealthReport['pressure']): string {
  switch (pressure) {
    ...
    case 'near-limit': return 'is-warn';
    case 'storage-pressure': return 'is-breach';
    case 'unknown': return 'is-muted';
  }
}
...
export function StorageCleanupDialog(props: StorageCleanupDialogProps) {
  ...
  async function handleRequestPersist() {
    ...
    setPersistStatus(granted ? 'Persistent storage granted.' : 'Persistent storage request denied.');
    props.onRefresh();
  }
    ...
    <Show when={props.open}>
      ...
      >
        <header class="capability-panel-header">
          <div>
            <p class="panel-title" id="storage-cleanup-title">Storage Cleanup</p>
          ...
          <Button size="icon" variant="ghost" onClick={props.onClose} aria-label="Close storage cleanup">
            <X size={16} aria-hidden="true" />
          </Button>
        </header>

        <Show when={props.report} fallback={<p class="capability-panel-note">Loading storage report...</p>}>
          {(report) => (
            <>
              <section class="diagnostics-section">
                <h2>Health</h2>
                <dl class="diagnostics-grid">
                  <div><dt>IndexedDB</dt><dd>{report().indexedDbHealthy ? 'healthy' : 'error'}</dd></div>
                  <div><dt>OPFS</dt><dd>{report().opfsAvailable ? 'available' : 'unavailable'}</dd></div>
                  <div><dt>Persistent</dt><dd>{report().persistentStorage}</dd></div>
                </dl>
                ...
                <Show when={persistStatus()}>
                  <p class="diagnostics-copy-status" aria-live="polite">{persistStatus()}</p>
                </Show>
              </section>
                          ...
                          <Show when={result()}>
                            {(r) => (
                              <p class={r().ok ? 'is-ok' : 'is-breach'}>
                                {r().ok ? `Done (freed ${formatBytes(r().freedBytes)})` : `Error: ${r().error}`}
                              </p>
                            )}
                          </Show>
                        </li>
