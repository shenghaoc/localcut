...
export function SilenceReviewPanel(props: SilenceReviewPanelProps) {
  ...
  const [openThreshold, setOpenThreshold] = createSignal(
    SILENCE_DEFAULTS.openThreshold,
  );
  const [closeThreshold, setCloseThreshold] = createSignal(
    SILENCE_DEFAULTS.closeThreshold,
  );
  const [minSilence, setMinSilence] = createSignal(SILENCE_DEFAULTS.minSilence);
  const [keepPadding, setKeepPadding] = createSignal(
    SILENCE_DEFAULTS.keepPadding,
  );
  const [minKeptSegment, setMinKeptSegment] = createSignal(
    SILENCE_DEFAULTS.minKeptSegment,
  );
  ...
  function handleApplyAll() {
    ...
    const allApplied = new Set<number>(
      r.map((_, i) => i).filter((i) => !s.has(i)),
    );
    setApplied(
      (prev: Set<number>) => new Set<number>([...prev, ...allApplied]),
    );
  }
    ...
    >
      ...
      >
        ...
        <div class="silence-params">
          ...
          <label>
            ...
            />
            <span class="silence-param-value">
              {minKeptSegment().toFixed(1)}
            </span>
          </label>
        </div>
      </details>
      ...
      <div class="silence-actions">
        <Button
          ...
          title={
            audioDisabled()
              ? "Select at least one audio track to detect silence."
              : undefined
          }
        >
