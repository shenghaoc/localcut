...
export function VoiceCleanupPanel(props: VoiceCleanupPanelProps) {
  ...
  const currentTarget = () =>
    useCustomTarget()
      ? customTargetLufs()
      : props.settings.normalisationTargetLufs;
        ...
        <div class="collapse-body" id="voice-cleanup-body">
          ...
          >
            <div class="denoiser-tracks">
              ...
              <Show when={props.trackNames.size > 0}>
                <p class="insert-hint">
                  Enable per-track denoising. The denoiser runs on the monitor
                  bus and export chain.
                </p>
              </Show>
              <Show when={props.denoiserStatus === "unavailable"}>
                <div class="analysis-error" role="alert">
                  ...
                  <span>
                    Denoiser unavailable: {props.denoiserUnavailableReason}
                  </span>
                </div>
              </Show>
              ...
              <For each={[...props.trackNames.entries()]}>
                {([trackId, name]) => (
                  <label class="track-toggle">
                    <input
                      type="checkbox"
                      checked={props.settings.denoiserEnabledTracks.includes(
                        trackId,
                      )}
          ...
          >
            <div class="loudness-controls">
              ...
              <Show when={props.settings.normaliseGainDb !== 0}>
                <p class="insert-hint">
                  Active correction:{" "}
                  {props.settings.normaliseGainDb >= 0 ? "+" : ""}
          ...
          <AudioInsertRow
            ...
            onToggleBypass={() =>
              updateLimiterParams({ bypass: !limiterBypass() })
            }
          >
            <SliderControl
              label="Ceiling"
