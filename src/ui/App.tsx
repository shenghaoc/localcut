...
} from "../engine/live-audio/live-chain";
...
import {
  AudioCleanupPanel,
  type AppliedCleanupInfo,
} from "./AudioCleanupPanel";
...
import {
  SmartReframePanel,
  type ReframeAnalyseSettings,
} from "./SmartReframePanel";
import { ReframeOverlay } from "./ReframeOverlay";
import {
  ReframeController,
  type ReframeControllerState,
} from "./reframe-controller";
...
const MEDIA_FILE_PATTERN =
  /\.(mp4|mov|webm|png|jpe?g|webp|gif|bmp|avif|mp3|m4a|wav|ogg|json)$/i;
...
function capabilityTierV2Label(
...
export function App() {
  const [capabilities, setCapabilities] =
    createSignal<CapabilitySnapshot>(probeCapabilities());
  const [capabilityProbeV2, setCapabilityProbeV2] =
    createSignal<CapabilityProbeResult | null>(null);
  ...
  const [previewBackend, setPreviewBackend] =
    createSignal<PreviewBackend>("none");
  ...
  const [docsSlug, setDocsSlug] = createSignal<string | null>(
    typeof window === "undefined"
      ? null
      : parseDocsPath(window.location.pathname),
  );
  ...
  const [convertOpen, setConvertOpen] = createSignal<boolean>(
    typeof window === "undefined"
      ? false
      : parseConvertPath(window.location.pathname),
  );
  const [publishState, setPublishState] = createSignal<PublishState>({
    phase: "idle",
  });
  const [publishTapStats, setPublishTapStats] =
    createSignal<PublishTapStats | null>(null);
  const [publishErrorDetail, setPublishErrorDetail] = createSignal<
    string | null
  >(null);
  ...
  const [diagnosticSnapshot, setDiagnosticSnapshot] =
    createSignal<DiagnosticSnapshot | null>(null);
  const [recentErrorLog, setRecentErrorLog] = createSignal(
    createEmptyRecentErrorLog(),
  );
  ...
  const [previewCanvasEl, setPreviewCanvasEl] = createSignal<
  ...
  const [coverFrame, setCoverFrame] = createSignal<
    import("../protocol").CoverFrameDoc | null
  >(null);
  const [coverTitleClipId, setCoverTitleClipId] = createSignal("");
  const [coverThumbnailUrl, setCoverThumbnailUrl] = createSignal<string | null>(
    null,
  );
  const [coverThumbnailError, setCoverThumbnailError] = createSignal<
    string | null
  >(null);
  const [_coverExportError, setCoverExportError] = createSignal<string | null>(
    null,
  );
  ...
  const selectedPlatform = createMemo<
    import("../engine/safe-zones").SafeZonePlatform | null
  >(() => {
  ...
  const matchingPlatforms = createMemo(() => {
    return (
      safeZoneFile()?.platforms.filter((p) => p.aspect === projectAspect()) ??
      []
    );
  });
  ...
  const [calloutPlacementActive, setCalloutPlacementActive] =
    createSignal(false);
  const [calloutPlacementKind, setCalloutPlacementKind] =
    createSignal<CalloutKind>("arrow");
  ...
  const [captionTracks, setCaptionTracks] = createSignal<
    CaptionTrackSnapshot[]
  >([]);
  const [sessionEventLogs, setSessionEventLogs] = createSignal<
    SessionEventLogRef[]
  >([]);
  const [captionDiagnostics, setCaptionDiagnostics] = createSignal<
  ...
  const [transitions, setTransitions] = createSignal<
    TimelineTransitionSnapshot[]
  >([]);
  ...
  >();
  const [selectedTransitionId, setSelectedTransitionId] = createSignal<
    string | null
  >(null);
  ...
  const [selectedClipRefs, setSelectedClipRefs] = createSignal<
    TimelineClipReference[]
  >([]);
  const [selectedCaptionTrackId, setSelectedCaptionTrackId] = createSignal<
    string | null
  >(null);
  const [selectedCaptionSegmentIds, setSelectedCaptionSegmentIds] =
    createSignal<string[]>([]);
  const [timelineClipboard, setTimelineClipboard] = createSignal<
    TimelineClipboardClip[]
  >([]);
  const [waveformPeaks, setWaveformPeaks] = createSignal<
    Record<string, WaveformPeaks>
  >({});
  const [exporting, setExporting] = createSignal(false);
  const [exportProgress, setExportProgress] =
    createSignal<ExportProgress | null>(null);
  ...
  const [exportSettings, setExportSettings] =
    createSignal<ExportSettings | null>(null);
  ...
  const [interpolationModelSizeBytes, setInterpolationModelSizeBytes] =
    createSignal<number | null>(null);
  const [interpolationModelCacheSource, setInterpolationModelCacheSource] =
    createSignal<"cache" | "network" | null>(null);
  const [interpolationEstimateMs, setInterpolationEstimateMs] = createSignal<
    number | null
  >(null);
  const [interpolationRefusals, setInterpolationRefusals] = createSignal(0);
  const [interpolationRecentErrors, setInterpolationRecentErrors] =
    createSignal<readonly string[]>([]);
  // Phase 32b: beauty model load state (mirrors interpolation; drives Inspector gating).
  const [beautyModelStatus, setBeautyModelStatus] =
    createSignal<BeautyModelStatus>("not-loaded");
  const [beautyModelSizeBytes, setBeautyModelSizeBytes] = createSignal<
    number | null
  >(null);
  const [beautyModelDownloadedBytes, setBeautyModelDownloadedBytes] =
    createSignal<number | null>(null);
  const [beautyModelError, setBeautyModelError] = createSignal<string | null>(
    null,
  );
  ...
  const [sidecarReadySessionId, setSidecarReadySessionId] = createSignal<
    string | null
  >(null);
  const [recorderLandedSessionId, setRecorderLandedSessionId] = createSignal<
    string | null
  >(null);
  const [retakeClipId, setRetakeClipId] = createSignal<string | null>(null);
  const [replayBufferState, setReplayBufferState] =
    createSignal<RingBufferState | null>(null);
  const [replaySaveInProgress, setReplaySaveInProgress] = createSignal(false);
  const [programSources, setProgramSources] = createSignal<
  ...
  >("idle");
  const [programActiveSceneId, setProgramActiveSceneId] = createSignal<
    string | null
  >(null);
  const [programSourceStatus, setProgramSourceStatus] = createSignal<
    ProgramSourceStatusSnapshot[]
  >([]);
  const [programError, setProgramError] = createSignal<string | null>(null);
  const [programTransitionMs, setProgramTransitionMs] = createSignal<0 | 200>(
    0,
  );
  ...
  const [liveChainConfig, setLiveChainConfig] =
    createSignal<LiveAudioChainConfig>(DEFAULT_LIVE_AUDIO_CHAIN_CONFIG);
  const [liveChainLatencyMs, setLiveChainLatencyMs] = createSignal(0);
  // Phase 36: Voice Cleanup
  const [voiceCleanupSettings, setVoiceCleanupSettings] =
    createSignal<VoiceCleanupSettings>(DEFAULT_VOICE_CLEANUP_SETTINGS);
  const [voiceCleanupAnalysisState, setVoiceCleanupAnalysisState] =
    createSignal<"idle" | "running" | "done" | "error">("idle");
  const [voiceCleanupAnalysisProgress, setVoiceCleanupAnalysisProgress] =
    createSignal(0);
  const [voiceCleanupMeasuredLufs, setVoiceCleanupMeasuredLufs] =
    createSignal(0);
  const [voiceCleanupProposedGainDb, setVoiceCleanupProposedGainDb] =
    createSignal(0);
  const [voiceCleanupNormalisedLufs, setVoiceCleanupNormalisedLufs] =
    createSignal(0);
  const [voiceCleanupAnalysisError, setVoiceCleanupAnalysisError] =
    createSignal("");
  const [voiceCleanupDenoiserStatus, setVoiceCleanupDenoiserStatus] =
    createSignal<"idle" | "loading" | "ready" | "unavailable">("idle");
  const [
    voiceCleanupDenoiserUnavailableReason,
    setVoiceCleanupDenoiserUnavailableReason,
  ] = createSignal("");
  const [voiceCleanupWasmSha256, setVoiceCleanupWasmSha256] = createSignal<
    string | null
  >(null);
  const [voiceCleanupWasmLoadTimeMs, setVoiceCleanupWasmLoadTimeMs] =
    createSignal<number | null>(null);
  const [voiceCleanupMonitorSampleRate, setVoiceCleanupMonitorSampleRate] =
    createSignal(48_000);
  const [voiceCleanupMonitorLatencyMs, setVoiceCleanupMonitorLatencyMs] =
    createSignal(voiceCleanupLatencyMs(48_000));
  ...
  const [restoreOffer, setRestoreOffer] =
    createSignal<RestoreOfferState | null>(null);
  const [unresolvedSources, setUnresolvedSources] = createSignal<
  ...
  >(new Map());
  const [beatProgress, setBeatProgress] = createSignal<Map<string, number>>(
    new Map(),
  );
  ...
  const [bundleReport, setBundleReport] =
    createSignal<BundleIntegrityReportSnapshot | null>(null);
  const initialSideRail = readInitialSideRailState();
  const [activeSideRailTab, setActiveSideRailTab] = createSignal<SideRailTab>(
    initialSideRail.tab,
  );
  ...
  const [sideRailCollapsed, setSideRailCollapsed] = createSignal(
    initialSideRail.collapsed,
  );
  const toggleSideRail = (collapsed: boolean) => {
    ...
    queueMicrotask(() => {
      if (collapsed) {
        ...
        document
          .getElementById(sideRailTabTriggerId(activeSideRailTab()))
          ?.focus();
      }
    });
  };
  ...
  const textSideRailTabs = createMemo(() =>
    visibleTextSideRailTabs(languageToolsVisible()),
  );
  createEffect(() => {
    if (
      activeTextSideRailTab() === "language-tools" &&
      !languageToolsVisible()
    ) {
  ...
  createEffect(() => {
    if (
      activeSideRailTab() === "capture" &&
      activeCaptureSideRailTab() === "publish"
    ) {
  ...
  const [interchangeWarnings, setInterchangeWarnings] = createSignal<
    readonly string[]
  >([]);
  const [interchangeMessage, setInterchangeMessage] = createSignal<
    string | null
  >(null);
  const [thumbnailVersion, setThumbnailVersion] = createSignal(0);
  const thumbnailStore = new ThumbnailStore();

  const unresolvedIds = createMemo(
    () => new Set(unresolvedSources().map((s) => s.sourceId)),
  );
  ...
  async function loadVerifiedVoiceCleanupWasm(): Promise<ArrayBuffer> {
    ...
    if (!manifestResponse.ok) {
      throw new Error(
        `RNNoise manifest fetch failed with HTTP ${manifestResponse.status}`,
      );
    }
    if (!wasmResponse.ok) {
      throw new Error(
        `RNNoise WASM fetch failed with HTTP ${wasmResponse.status}`,
      );
    }
    ...
    if (actual !== expected) {
      throw new Error(
        `RNNoise WASM checksum mismatch: expected sha256-${expected}`,
      );
    }
  ...
  const [activeDockTab, setActiveDockTab] = createSignal<"media" | "beats">(
    "media",
  );

  createEffect(() => {
    const snapAvailable =
      beatResults().size > 0 && beatSettings().enabledSourceIds.length > 0;
  ...
  const [translationState, setTranslationState] =
    createSignal<TranslationControllerState>(translationController.getState());
  ...
  async function refreshLanguageToolsProbe(): Promise<void> {
    ...
    setCapabilityProbeV2((prev) =>
      prev ? { ...prev, languageTools: result } : prev,
    );
  }

  function exportBilingualCaptions(
  ...
  function settleSourceFile(
    requestId: string,
    action: (pending: PendingSourceFile) => void,
  ): void {
  ...
  function requestSourceFile(sourceId: string): Promise<File> {
    if (!bridge)
      return Promise.reject(new Error("Media pipeline is not ready."));
    ...
    return new Promise<File>((resolve, reject) => {
      const timer = setTimeout(() => {
        settleSourceFile(requestId, (pending) =>
          pending.reject(
            new Error(
              "Timed out resolving the source media for Smart Reframe.",
            ),
          ),
        );
  ...
  const selectedReframeClip = createMemo<ReframeClipTarget | null>(() => {
    for (const ref of selectedClipRefs()) {
      ...
      const video = assets().find(
        (item) => item.sourceId === clip.sourceId,
      )?.video;
      if (!video) continue;
      ...
      return {
        ...
        hasKeyframes: Boolean(
          kf?.x?.length || kf?.y?.length || kf?.scale?.length,
        ),
  ...
  async function handleReframeAnalyse(settings: ReframeAnalyseSettings) {
    ...
    const sourceAspect =
      clip.sourceHeight > 0 ? clip.sourceWidth / clip.sourceHeight : 1;
  ...
  function findTimelineClip(
    ref: TimelineClipReference,
  ): TimelineClipSnapshot | null {
    const track = timeline().find((item) => item.id === ref.trackId);
  ...
  const selectedTitle = createMemo(() => {
    ...
    if (!track || track.type !== "video") return null;
    const timelineClip = track.clips.find((c) => c.id === clip.clipId);
    if (!timelineClip || timelineClip.kind !== "title" || !timelineClip.title)
      return null;
    return {
      trackId: track.id,
      clipId: timelineClip.id,
  ...
  const pipelineLabel = createMemo(() => {
    switch (previewBackend()) {
      ...
      case "none":
        ...
        return capabilityProbeV2()?.tier === "shell-only"
          ? "Shell only"
          : "Limited shell";
    }
  });
  ...
  const replayCaptureUnsupportedReason = () => {
    ...
    if (!workerReady())
      return "Replay Buffer is unavailable until the pipeline worker is ready.";
    return null;
  };
  ...
  function recordInterpolationError(message: string): void {
    setInterpolationRecentErrors((current) =>
      [message, ...current].slice(0, 5),
    );
  }

  async function refreshDiagnostics(
    workerSnapshot?: DiagnosticSnapshot | null,
  ) {
    const snapshot = await buildUiDiagnosticSnapshot({
      ...
      recentErrors: (() => {
        ...
        const workerCodes = new Set(
          workerLog.entries.map((w) => `${w.subsystem}:${w.code}`),
        );
        ...
        return {
          ...workerLog,
          entries: [...workerLog.entries, ...uiOnly].slice(
            0,
            workerLog.capacity,
          ),
        };
      ...
      voiceCleanup: {
        denoiserEnabledTrackCount:
          voiceCleanupSettings().denoiserEnabledTracks.length,
        wasmLoadStatus:
          voiceCleanupDenoiserStatus() === "ready"
            ? "loaded"
  ...
  async function startReplayCapture() {
    ...
    try {
      ...
      const lifetimeTrack =
        streams.mediaStream.getVideoTracks()[0] ??
        streams.mediaStream.getAudioTracks()[0];
  ...
  function sanitizedProgramSources(): ProgramSourceDescriptor[] {
    return [...programSourceHandles.values()].map(({ descriptor }) => ({
      ...descriptor,
      track: null,
      encoderConfig: descriptor.encoderConfig
        ? { ...descriptor.encoderConfig }
        : null,
    }));
  }
  ...
  function programLayerForSource(
    source: ProgramSourceDescriptor,
    zIndex: number,
  ): SceneLayer {
    return {
  ...
  function updateProgramScenes(
    mutator: (scenes: SceneDefinition[]) => SceneDefinition[],
  ): void {
  ...
  function ensureProgramSceneForSource(source: ProgramSourceDescriptor): void {
    ...
    updateProgramScenes((prev) => {
      ...
      return prev.map((scene) => ({
        ...scene,
        layers: scene.layers.some(
          (layer) => layer.sourceRef === source.sourceId,
        )
          ? scene.layers
          : [
              ...scene.layers,
              programLayerForSource(source, scene.layers.length),
            ],
      }));
    });
  }
  ...
  function releasePendingProgramSources(stopTracks = true): void {
    if (stopTracks) {
      for (const {
        descriptor,
        monitorTrack,
      } of programSourceHandles.values()) {
  ...
  async function addProgramScreen(): Promise<void> {
    try {
      ...
      monitorTrack.addEventListener(
        "ended",
        () => removeProgramSource(sourceId),
        { once: true },
      );
      addProgramSource(
        {
    ...
    updateProgramScenes((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          name: `Scene ${index + 1}`,
          hotkey:
            index < 9 ? (`${index + 1}` as SceneDefinition["hotkey"]) : null,
          layers: sources.map((source, sourceIndex) =>
            programLayerForSource(source, sourceIndex),
          ),
        },
      ];
    });
  ...
  function setProgramSceneHotkey(sceneId: string, hotkey: string | null): void {
    const nextHotkey = /^[1-9]$/.test(hotkey ?? "")
      ? (hotkey as SceneDefinition["hotkey"])
      : null;
    updateProgramScenes((prev) =>
      prev.map((scene) => ({
        ...scene,
  ...
  function updateProgramSceneLayers(
    sceneId: string,
    layers: SceneLayer[],
  ): void {
    updateProgramScenes((prev) =>
      prev.map((scene) =>
        scene.id === sceneId ? { ...scene, layers } : scene,
      ),
    );
  }
  ...
  function startProgramSession(initialSceneId: string): void {
    ...
    const sources = [...programSourceHandles.values()].map(
    ...
    for (const [sourceId, handle] of programSourceHandles.entries()) {
      if (handle.monitorTrack)
        monitorEntries.push([sourceId, handle.monitorTrack]);
    }
  ...
  function requestPreviewRegionPick(
  ...
  function handleState(msg: WorkerStateMessage) {
    ...
    switch (msg.type) {
      ...
      case "source-file-error":
        settleSourceFile(msg.requestId, (pending) =>
      ...
      case "interp-model-status":
        ...
        if (msg.sizeBytes !== undefined)
          setInterpolationModelSizeBytes(msg.sizeBytes);
      ...
      case "beauty-model-status":
        ...
        if (msg.downloadedBytes !== undefined)
          setBeautyModelDownloadedBytes(msg.downloadedBytes);
        if (msg.error) setBeautyModelError(msg.error);
        else if (msg.status === "loaded" || msg.status === "not-loaded")
          setBeautyModelError(null);
        break;
      ...
      case "time-remap-updated":
        setStatusLine(
          `Speed ramp applied (new duration: ${msg.outputDurationS.toFixed(2)}s)`,
        );
      ...
      case "import-progress":
        setImporting(true);
        setStatusLine(
          msg.stage === "reading" ? "Reading file…" : "Extracting metadata…",
        );
        break;
      ...
      case "media-assets": {
        ...
        setLatestHealthReport((prev) =>
          prev && msg.assets.some((asset) => asset.sourceId === prev.sourceId)
            ? null
            : prev,
        );
      ...
      case "queue-job-progress":
        setRenderQueue((prev) => ({
          ...
          jobs: prev.jobs.map((j) =>
            j.id === msg.jobId ? { ...j, progress: msg.progress } : j,
          ),
        }));
      ...
      case "queue-job-complete":
        setStatusLine(
          `Queue: ${msg.fileName} done (${Math.round(msg.elapsedSeconds)}s)`,
        );
        break;
      ...
      case "diagnostic-snapshot": {
        setRecentErrorLog((prev) => {
          ...
          const workerCodes = new Set(
            workerEntries.map((e) => `${e.subsystem}:${e.code}`),
          );
          const uiOnly = prev.entries.filter(
            (e) => !workerCodes.has(`${e.subsystem}:${e.code}`),
          );
      ...
      case "recovery-state":
        if (msg.state === "recovering") {
          ...
          setExportReady(false);
          setRuntimeIssue(
            "GPU recovery failed. Accelerated features are unavailable.",
          );
      ...
      case "source-health": {
        const visibleReport =
          msg.report.status === "ok"
            ? null
            : userVisibleHealthReport(msg.report);
      ...
      case "bundle-import-result":
        ...
        setBundleMessage(
          msg.reason ??
            (msg.ok ? "Bundle job complete." : "Bundle job failed."),
        );
      ...
      case "replay-save-progress":
        ...
        setStatusLine(
          `Saving replay… ${msg.chunksWritten}/${msg.totalChunks} chunks`,
        );
        break;
      ...
      case "cover-thumbnail":
        {
          ...
          if (
            ...
            (currentCover.titleClipId ?? null) ===
              (msg.cover.titleClipId ?? null)
          ) {
    ...
    setRecentErrorLog((prev) =>
      addRecentError(
        ...
        createRecentError({
          ...
          message,
          recoveryActionIds:
            crashState === "throttled" ? [] : ["restart-worker"],
        }),
      ),
    );
    setStatusLine(
  ...
  async function sendInit(canvas: OffscreenCanvas) {
    ...
    b.send(
      { type: "init", canvas, sab, audioSab, scopeSab, probeResult: probe },
      [canvas],
    );
  }
  ...
  async function pickOutputHandle(
  ...
  async function handleQueueJobDestination(
    jobId: string,
  ...
  async function preselectQueueOutputHandles(): Promise<boolean> {
    const pendingJobs = renderQueue().jobs.filter(
  ...
  function rejectUnavailableInterpolationExport(
    settings: ExportSettings,
  ): boolean {
  ...
  function selectClip(
    ...
    if (!additive) {
      setSelectedClipRefs((prev) =>
        prev.some((ref) => `${ref.trackId}:${ref.clipId}` === key)
          ? prev
          : [next],
      );
      return;
  ...
  function copySelectedClips() {
    ...
    setStatusLine(
      `Copied ${clips.length} clip${clips.length === 1 ? "" : "s"}`,
    );
  }
  ...
  function splitKeyframedTransformChange(
    ...
    for (const [rawKey, value] of Object.entries(transform)) {
      ...
      if (!hasKeyframeTrack(sel.keyframes, key)) {
        staticNumbers[
          key as Exclude<keyof TimelineClipSnapshot["transform"], "fit">
        ] = value;
        continue;
      }
  ...
  function zoomTimeline(direction: 1 | -1) {
    window.dispatchEvent(
      new CustomEvent("localcut-timeline-zoom", { detail: { direction } }),
    );
  }
  ...
  onMount(() => {
    ...
    onCleanup(() => {
      ...
      if (worker && bridge) {
        ...
        const cleanup = {
          terminateFallback: undefined as
      ...
      >
            ...
            return (
              ...
              >
                <div class="bundle-replace-modal">
                  <p
                    id="bundle-replace-title"
                    class="bundle-replace-modal-title"
                  >
                    Replace current project?
                  </p>
                  <p
                    id="bundle-replace-message"
                    class="bundle-replace-modal-message"
                  >
                    {prompt.message}
                  </p>
          ...
          >
                ...
                fallback={
                  <>
                    ...
                    <p class="restore-banner-detail">
                      ...
                      {unresolvedSources().length === 1 ? "" : "s"} need
                      re-linking.
                    </p>
                  </>
                }
                ...
                {(offer) => (
                  <>
                    ...
                    <p class="restore-banner-detail">
                      ...
                      {offer().sources.length === 1 ? "" : "s"} in the saved
                      project.
                    </p>
                  </>
                )}
                ...
                <For each={unresolvedSources()}>
                  {(source) => (
                    <li class="restore-source-item">
                      <span title={formatSourceSummary(source)}>
            ...
            <Show when={restoreOffer()}>
              <div class="restore-actions">
                <Button
                  onClick={() => bridge?.send({ type: "restore-project" })}
                >
        ...
        <AppErrorBoundary>
          ...
          >
                ...
                <div class="dock-library">
                  ...
                  >
                    <MediaBin
                      ...
                      onPlace={(sourceId) =>
                        bridge?.send({ type: "place-clip", sourceId })
                      }
                      onRemove={(sourceId) =>
                        bridge?.send({ type: "remove-asset", sourceId })
                      }
                    />
                  </SecondaryRailPanel>
                  <SecondaryRailPanel
                  ...
                  >
                    <BeatPanel
                      ...
                      onAnalyse={(sourceId) =>
                        bridge?.send({ type: "analyze-beats", sourceId })
                      }
                      onCancel={(sourceId) => {
                        bridge?.send({
                      ...
                      onToggleSource={(sourceId, enabled) => {
                        ...
                        const ids = enabled
                          ...
                          : current.enabledSourceIds.filter(
                              (id) => id !== sourceId,
                            );
                        bridge?.send({
            ...
            >
              <Show when={previewKey() + 1} keyed>
                {(_k) => (
                  <PreviewCanvas
                    onOffscreenReady={sendInit}
                    onCanvasEl={setPreviewCanvasEl}
                  />
                )}
              </Show>
              <Show
              ...
              >
                <PreviewGizmo
                  ...
                  onChange={(transform) => {
                    ...
                    if (!sel) return;
                    const staticPatch =
                      splitKeyframedTransformChange(transform);
              ...
              >
                ...
                >
                  <p>
                    Drag on the preview to set the zoom region. Press Escape to
                    cancel.
                  </p>
                </div>
              </Show>
              <Show
                when={previewSurfaceAvailable() && calloutPlacementActive()}
              >
                <div
                  ...
                  onPointerDown={(e) => {
                    ...
                    const onPointerUp = (upEvent: PointerEvent) => {
                      ...
                      completeCalloutPlacement(
                        calloutPlacementKind(),
                        startX,
                        startY,
                        endX,
                        endY,
                ...
                >
                  <p>
                    Drag to place {calloutPlacementKind()} callout. Press Escape
                    to cancel.
                  </p>
                </div>
              </Show>
                ...
                <div class="phase39-controls">
                    ...
                    <For each={["16:9", "9:16", "1:1", "4:5"] as const}>
                      {(aspect) => (
                        <button
                          ...
                          onClick={() =>
                            bridge?.send({ type: "set-project-format", aspect })
                          }
                        >
                          {aspect}
                        </button>
                  ...
                  <Show when={matchingPlatforms().length > 0}>
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedPlatformId(e.currentTarget.value)
                      }
                      style={{
                  ...
                  <Show when={coverTitleOptions().length > 0}>
                    <select
                      ...
                      onChange={(e) =>
                        setCoverTitleClipId(e.currentTarget.value)
                      }
                  ...
                  <Show when={coverThumbnailError()}>
                    {(error) => (
                      <span class="cover-thumb-warning">{error()}</span>
                    )}
                  </Show>
                </div>
              </Show>
              ...
              >
                <Tabs.Root
                  ...
                  onValueChange={(details) => {
                    if (isSideRailTab(details.value))
                      setActiveSideRailTab(details.value);
                  }}
                >
                  <Tabs.List
                    class="side-rail-tab-bar"
                  ...
                  <div class="side-rail-tab-content">
                    ...
                    >
                      <Inspector
                        ...
                        beautyModelSizeBytes={
                          beautyModelSizeBytes() ?? undefined
                        }
                        beautyModelDownloadedBytes={
                          beautyModelDownloadedBytes() ?? undefined
                        }
                        ...
                        >
                          <LanguageToolsPanel
                            ...
                            onGenerateDraft={(trackId) => {
                              ...
                              if (!track) return;
                              pauseFromKeyboard();
                              void draftController.generateDraft(
                                track.segments,
                              );
                            }}
                    ...
                    >
                      ...
                      >
                        <RecordPanel
                          ...
                          onPushFrame={(sourceId, frame) => {
                            ...
                            const inFlight =
                              capturePushInFlight.get(sourceId) ?? 0;
                            ...
                            // successful transfer.
                            bridge.send(
                              { type: "capture-push-frame", sourceId, frame },
                          ...
                          onPause={() =>
                            bridge?.send({ type: "capture-pause" })
                          }
                          onResume={() =>
                            bridge?.send({ type: "capture-resume" })
                          }
                      ...
                      >
                        <ProgramPanel
                          ...
                          onAddCamera={(deviceId) =>
                            void addProgramCamera(deviceId)
                          }
                      ...
                      >
                        <PublishPanel
                          ...
                          onGoLive={(settings) =>
                            void publishController.goLive(settings)
                          }
          ...
          <Timeline
            ...
            onSplit={(trackId, _clipId, time) =>
              bridge?.send({ type: "split", trackId, time })
            }
            onDelete={(trackId, clipId) =>
              bridge?.send({ type: "delete-clip", trackId, clipId })
            }
            ...
            onAddMarker={(time, label) =>
              bridge?.send({ type: "add-marker", time, label })
            }
            onDeleteMarker={(markerId) =>
              bridge?.send({ type: "delete-marker", markerId })
            }
            onCloseGaps={(trackId) =>
              bridge?.send(
                trackId
                  ? { type: "close-gaps", trackId }
                  : { type: "close-gaps" },
              )
            }
            ...
            onAddTrack={(trackType) =>
              bridge?.send({ type: "add-track", trackType })
            }
            onRemoveTrack={(trackId) =>
              bridge?.send({ type: "remove-track", trackId })
            }
          ...
          <RenderQueuePanel
            ...
            onCancelJob={(jobId) =>
              bridge?.send({ type: "queue-cancel-job", jobId })
            }
          ...
          <footer class="status-bar">
            ...
            <span class="status-meta">
              ...
              <Show when={workerRecoveryState() !== "running"}>
                <span
                  class="status-badge status-warn"
                  title={`Worker: ${workerRecoveryState()}`}
                >
              ...
              <Show when={capabilityTierV2Label(capabilityProbeV2())}>
                {(label) => (
                  <span
                    ...
                    title={
                      capabilityProbeV2()
                        ? (compatibilityReadiness(capabilityProbeV2()!.tier)
                            .note ?? "CapabilityTierV2")
                        : "CapabilityTierV2"
                    }
                  >
                    {label()}
          ...
          <AutoCaptionsPanel
            ...
            onTranscribeRange={(language) => {
              ...
              void asrController.transcribeRange(
                { startS, durationS },
                language,
              );
            }}
          ...
          <SmartReframePanel
            ...
            workerAvailable={
              capabilityProbeV2()?.smartReframe?.analysisWorker !==
              "unsupported"
            }
            onLoadFaceModel={() =>
            ...
            onClose={() => {
              ...
              if (status === "resolving" || status === "analysing")
                reframeController.cancel();
          ...
          <Show when={silenceReviewOpen()}>
            {(() => {
              ...
              const selectedAudioTrackIds = () => {
                ...
                for (const ref of selectedClipRefs())
                  selectedTracks.add(ref.trackId);
                return timeline()
              ...
              return (
                <SilenceReviewPanel
                  ...
                  selectionScope={
                    selectedAudioTrackIds().length > 0
                      ? "selection"
                      : "all-audio"
                  }
                  ...
                  onWorkerMessage={(handler) => {
                    const wrapped = (
                      msg: import("../protocol").WorkerStateMessage,
                    ) => {
                      if (
          ...
          <Show when={keystrokeOverlayOpen()}>
            <KeystrokeOverlayPanel
              ...
              sidecarReady={
                sidecarReadySessionId() === recorderLandedSessionId()
              }
          ...
          <CapabilityPanel
            ...
            compatibilityPreviewAvailable={canCompatibilityPreview(
              capabilities(),
            )}
                  ...
                  if (sab) {
                    ...
                    audioReady.then(
                      (result) => {
                        ...
                        setVoiceCleanupMonitorSampleRate(
