...
export interface BeatPanelProps {
  ...
  beatResults: () => ReadonlyMap<
    string,
    { tempoBpm: number; beatTimesMs: number[] }
  >;
...
export function BeatPanel(props: BeatPanelProps) {
  const audioSources = createMemo(() =>
    props.assets().filter((a) => a.audio != null),
  );

  const hasBeatData = createMemo(() => props.beatResults().size > 0);
  const hasEnabledSource = createMemo(
    () => props.beatSettings().enabledSourceIds.length > 0,
  );
      ...
      <div class="beat-panel-sources">
        <For each={audioSources()}>
          {(source) => {
            ...
            const progress = () =>
              props.analysisProgress().get(source.sourceId);
            const isEnabled = () =>
              props.beatSettings().enabledSourceIds.includes(source.sourceId);
              ...
              <div class="beat-panel-source-row">
                  ...
                  <Show
                    ...
                    fallback={
                      <Show
                        when={result()}
                        fallback={
                          <span class="beat-panel-state">Not yet run</span>
                        }
                      >
                ...
                <div class="beat-panel-source-controls">
                  ...
                  <button
                    ...
                    onClick={() =>
                      props.onToggleSource(source.sourceId, !isEnabled())
                    }
        ...
        <div class="beat-panel-global">
          <label class="beat-panel-offset-label">
            ...
            <input
              ...
              onInput={(e) =>
                props.onOffsetChange(Number(e.currentTarget.value))
              }
