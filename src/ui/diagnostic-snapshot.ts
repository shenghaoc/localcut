...
function browserSummary(): DiagnosticSnapshot['browser'] {
  ...
  if (chromium) {
    return { userAgentFamily: 'Chromium', userAgentVersion: chromium[1]!, platformFamily: navigator.platform };
  }
  if (firefox) {
    return { userAgentFamily: 'Firefox', userAgentVersion: firefox[1]!, platformFamily: navigator.platform };
  }
  if (safari) {
    return { userAgentFamily: 'Safari', userAgentVersion: safari[1]!, platformFamily: navigator.platform };
  }
  return { userAgentFamily: 'unknown', userAgentVersion: 'unknown', platformFamily: navigator.platform || 'unknown' };
}
...
function capabilityReport(
  ...
  const findings = [
    finding(
      ...
      snapshot.crossOriginIsolated ? undefined : 'Serve dev/preview/production with COOP/COEP headers, then reload.',
    ),
    finding(
      'capability.shared_array_buffer',
      snapshot.sharedArrayBuffer,
      snapshot.sharedArrayBuffer ? 'SharedArrayBuffer is available.' : 'SharedArrayBuffer is unavailable.',
    ...
    finding(
      ...
      snapshot.webCodecs ? undefined : 'Use a recent Chromium-based browser for accelerated import/export.',
    ),
  ];
  return {
    tier: diagnosticTier,
    ...
    webGpu: {
      ...
      features: [],
      optionalFeatures: {
        shaderF16: { code: 'webgpu.feature.shader-f16', status: 'unknown', message: 'Worker snapshot not available yet.' },
        timestampQuery: { code: 'webgpu.feature.timestamp-query', status: 'unknown', message: 'Worker snapshot not available yet.' },
        subgroups: { code: 'webgpu.feature.subgroups', status: 'unknown', message: 'Worker snapshot not available yet.' },
      },
    },
    webCodecs: {
      decoders: [{ codec: 'browser-reported', direction: 'decode', supported: snapshot.webCodecs }],
      encoders: [{ codec: 'browser-reported', direction: 'encode', supported: typeof VideoEncoder !== 'undefined' }],
    },
    mediabunny: { code: 'capability.mediabunny', status: 'supported', message: 'Mediabunny is bundled with the app.' },
    ...
    fileSystemAccess: finding(
      ...
      snapshot.fileSystemAccess ? 'File System Access pickers are available.' : 'File System Access pickers are unavailable.',
...
function proxyCacheSummary(assets: readonly MediaAssetSnapshot[]): ProxyCacheDiagnosticSummary {
  ...
  return {
    ...
    failedProxies,
    estimatedBytes,
    message: proxyAssets === 0 ? 'No proxy/cache assets are reported in the UI state.' : 'Proxy/cache status is available.',
  };
}
...
export async function buildUiDiagnosticSnapshot(input: UiDiagnosticInput): Promise<DiagnosticSnapshot> {
  ...
  return {
    ...
    capability: worker?.capability ?? capabilityReport(input.capabilities, input.tier, input.runtimeIssue, input.webgpuReady),
