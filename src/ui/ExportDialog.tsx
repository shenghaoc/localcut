...
export function ExportDialog(props: ExportDialogProps) {
  ...
  const [rangeMode, setRangeMode] = createSignal<"full" | "range" | "markers">(
    "full",
  );
  ...
  const supportedCodecSet = createMemo(
    () =>
      new Set(
        effectiveSupportedCodecs().map(
          (entry) => `${entry.codec}:${entry.container}`,
        ),
      ),
  );
  ...
  const platformCodecResolution = createMemo(() => {
    ...
    if (!preset || !preset.builtIn || preset.targetLufs === undefined)
      return null;
    const probe = props.capabilityProbeV2;
  ...
  function buildSettings(): ExportSettings {
    ...
    const range =
      mode === "range" && useRange() ? (buildRange() ?? undefined) : undefined;
    return {
      ...current,
          ...
          <Popover.Content class="export-popover panel" aria-label="Export">
            ...
            <Show when={props.presets.length > 0}>
              ...
              <div class="export-preset-selector">
                ...
                <Show
                  when={
                    selectedPresetId() &&
                    !props.presets.find((p) => p.id === selectedPresetId())
                      ?.builtIn
                  }
                >
                  <button
            ...
            <Show when={aspectMismatch()}>
              <p class="export-aspect-warning" role="alert">
                Export dimensions ({settings().width}×{settings().height}) do
                not match project format ({props.projectAspect}). The output may
                appear letterboxed.
              </p>
            </Show>
            <Show
            ...
            <Show
              when={
                platformCodecResolution() &&
                !("blocked" in platformCodecResolution()!) &&
                (platformCodecResolution() as { codec: ExportVideoCodec })
                  .codec !== settings().codec
              }
            >
              <p
                class="export-aspect-warning"
                role="status"
                aria-live="polite"
                aria-atomic="true"
              >
                H.264 is not supported on this device; falling back to VP9 in
                WebM container.
              </p>
            </Show>
              ...
              <For each={CODEC_OPTIONS}>
                {(entry) => {
                  const supported = createMemo(() =>
                    supportedCodecSet().has(
                      `${entry.codec}:${entry.container}`,
                    ),
                  );
                  return (
                    <button
                      ...
                      title={
                        supported()
                          ...
                          : disabledCodecReason(
                              entry.codec,
                              props.capabilityProbeV2,
                            )
                      }
                      onClick={() => setCodec(entry.codec)}
                    >
                      {codecLabel(entry.codec)} ·{" "}
            ...
            <Show when={nonCoreProbe()}>
              {(probe) => (
                <details class="export-tier-constraints" open>
                  ...
                  <p class="export-note">
                    {probe().tier} limits export to codecs and containers that
                    this browser can encode and mux.
                  </p>
                  <ul>
                    <For
                    ...
                    >
                      {(entry) => (
                        <li>
                          ...
                          <span>
                            {disabledCodecReason(entry.codec, probe())}
                          </span>
                        </li>
                      )}
                    </For>
                  ...
                  <button
                    ...
                    onClick={props.onWhyConstraints}
                  >
                    Why?
                  </button>
                </details>
                ...
                <Show when={settings().interpolation}>
                  <label class="export-field">
                    ...
                    <input
                      ...
                      max="240"
                      value={
                        settings().interpolation?.targetFps ??
                        Math.round(settings().fps * 2)
                      }
                      disabled={props.exporting}
                      onInput={(event) => {
                        setSettings((current) => ({
            ...
            <Show when={rangeMode() === "range"}>
              ...
              <Show
                when={
                  rangeError() ??
                  (rangeInvalid() ? "Out must be greater than In." : null)
                }
              >
            ...
            <Show when={rangeMode() === "markers"}>
              <p class="export-note">
                {props.markers.length} markers — {props.markers.length - 1}{" "}
                range
              ...
              {(progress) => (
                <div class="export-progress">
                  ...
                  <div class="export-estimate">
                    ...
                    <Show
                      when={
                        progress().subRealtime && progress().etaSeconds !== null
                      }
                    >
...
function ChaptersSection(props: {
    ...
    if (
      ...
      try {
        const handle = await (
          ...
          types: [
            { description: "Chapter Text", accept: { "text/plain": [".txt"] } },
          ],
        });
  ...
  return (
    <div class="chapters-section">
      <Show
        when={isValid()}
        fallback={<p class="export-error">{chapterReason()}</p>}
      >
