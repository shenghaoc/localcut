...
import { createShortcutRegistry, DEFAULT_SHORTCUTS, type ShortcutBinding } from './shortcut-registry';
...
describe('ShortcutRegistry', () => {
  ...
  it('resolves highest-priority scope', () => {
    const reg = createShortcutRegistry();
    const globalEsc: ShortcutBinding = { id: 'global-esc', key: 'escape', modifiers: [], scope: 'global', label: 'Global escape' };
    const dialogEsc: ShortcutBinding = { id: 'dialog-esc', key: 'escape', modifiers: [], scope: 'dialog', label: 'Close dialog' };
  ...
  it('dialog scope wins over timeline', () => {
    const reg = createShortcutRegistry();
    const timeline: ShortcutBinding = { id: 'timeline-esc', key: 'escape', modifiers: [], scope: 'timeline', label: 'Deselect' };
    const dialog: ShortcutBinding = { id: 'dialog-esc', key: 'escape', modifiers: [], scope: 'dialog', label: 'Close dialog' };
  ...
  it('text-entry scope keeps text-editing keys', () => {
    const reg = createShortcutRegistry();
    const globalS: ShortcutBinding = { id: 'split', key: 's', modifiers: [], scope: 'timeline', label: 'Split' };
    const textS: ShortcutBinding = { id: 'text-s', key: 's', modifiers: [], scope: 'text-entry', label: 'Type s' };
  ...
  it('respects when predicate', () => {
    const reg = createShortcutRegistry();
    let condition = false;
    reg.register({ id: 'conditional', key: 'x', modifiers: [], scope: 'global', label: 'Conditional', when: () => condition });
  ...
  it('Escape has one active meaning at a time', () => {
    const reg = createShortcutRegistry();
    reg.register({ id: 'esc-dialog', key: 'escape', modifiers: [], scope: 'dialog', label: 'Close' });
    reg.register({ id: 'esc-timeline', key: 'escape', modifiers: [], scope: 'timeline', label: 'Deselect' });
  ...
  it('default shortcuts have no duplicate chords in overlapping scopes', () => {
    ...
    for (const s of DEFAULT_SHORTCUTS) reg.register(s);
    const conflicts = reg.findConflicts();
    const nonBrowserConflicts = conflicts.filter((c) => !c.bindingA.browserReserved && !c.bindingB.browserReserved);
