...
function statusBadge(status: RenderQueueJob['status']): string {
  switch (status) {
    ...
    case 'choosing-destination': return 'Choosing file…';
...
function statusClass(status: RenderQueueJob['status']): string {
  switch (status) {
    case 'completed': return 'queue-status-done';
    case 'failed': return 'queue-status-failed';
    case 'canceled': return 'queue-status-canceled';
    case 'running':
    case 'finalizing': return 'queue-status-running';
...
export function RenderQueuePanel(props: RenderQueuePanelProps) {
  const pendingCount = createMemo(() => props.queue.jobs.filter((j) => j.status === 'pending').length);
  const completedCount = createMemo(() => props.queue.jobs.filter((j) => j.status === 'completed').length);
      ...
      <section class="render-queue-panel" aria-label="Render Queue">
        <div class="render-queue-header">
          <h3 class="render-queue-title">
            Render Queue <span class="text-xs text-[#9898a4] font-normal">(Experimental)</span>
              ...
              >
                <div class="render-queue-job-main">
                  <div class="render-queue-job-info">
                    ...
                    <span class="render-queue-job-range">
                      {jobRangeLabel(job.jobRange)}
                    </span>
                  ...
                  <div class="render-queue-job-status">
                    ...
                    <Show when={job.status === 'completed' && job.elapsedSeconds !== null}>
                      <span class="render-queue-job-elapsed">{formatElapsed(job.elapsedSeconds)}</span>
                    </Show>
                  </div>
                </div>
