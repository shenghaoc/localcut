...
function acceleratedSnapshot(): DiagnosticSnapshot {
  return {
    ...
    storage: { opfsSupported: true, indexedDbSupported: true, persistentStorage: 'granted', usageBytes: 1024, quotaBytes: 1_000_000, warning: 'ok' },
    proxyCache: { status: 'available', proxyAssets: 0, readyProxies: 0, failedProxies: 0, estimatedBytes: 0, message: 'No proxies' },
    activeExportSettings: { codec: 'h264', container: 'mp4', width: 1920, height: 1080, fps: 30, videoBitrate: 8_000_000, sourceMode: 'original', range: 'full' },
    performanceBudgets: [],
  ...
  return {
    ...
    webGpu: {
      ...
      optionalFeatures: {
        ...
        timestampQuery: { code: 'timestamp-query', status: 'unsupported', message: 'Not supported' },
    ...
    audioWorklet: { code: 'audio-worklet', status: 'unknown', message: 'Unknown in worker context' },
