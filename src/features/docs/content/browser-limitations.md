...
- **Optional on-device ML tools** such as Audio Cleanup and Auto Captions still need WebAssembly as the baseline. Audio Cleanup defaults to **ONNX Runtime DTLN** on the WASM (CPU) execution provider, and Auto Captions defaults to a quantized Whisper model on ONNX Runtime Web's WASM backend. The LiteRT DTLN and full-precision LiteRT caption models remain selectable alternates; LiteRT prefers experimental WebNN first, then WebGPU, and falls back to WASM. The panels show the engine and accelerator that actually loaded.
