...
async function trackStartAndDuration(track: InputTrack): Promise<{ startS: number; durationS: number | null }> {
  ...
  const endS = metadataEnd ?? await track.computeDuration({ skipLiveWait: true }).catch(() => null);
  return {
    startS,
...
async function inspectVideoTrack(track: InputVideoTrack): Promise<SourceVideoTrackInspection> {
  ...
  const frameRate = stats.averagePacketRate && stats.averagePacketRate > 0 ? stats.averagePacketRate : null;
  const [timing, codec, canDecode, codedWidth, codedHeight, displayWidth, displayHeight, rotationDeg, color] =
    await Promise.all([
      ...
      track.getRotation(),
      track.getColorSpace().then(colorHintsFromInit).catch(() => ({
        primaries: null,
        transfer: null,
        matrix: null,
        fullRange: null,
...
function deriveConformance(
  ...
  const firstVideo = inspection.tracks.find((track): track is SourceVideoTrackInspection => track.kind === 'video') ?? null;
  const firstAudio = inspection.tracks.find((track): track is SourceAudioTrackInspection => track.kind === 'audio') ?? null;
  const kind: MediaKind = hasDecodableVideo ? 'video' : hasDecodableAudio ? 'audio' : firstVideo ? 'video' : 'audio';
  ...
  const initial: SourceConformance = {
    ...
    timing,
    health:
      hasDecodableVideo || hasDecodableAudio
        ? 'ok'
        : 'blocked',
  };
...
export const mediabunnyAdapter: MediaAdapter = {
  ...
  async inspect(input: MediaAdapterOpenInput): Promise<MediaAdapterInspectionResult> {
    ...
    try {
      ...
      const { inspection, primaryVideoInspection, primaryAudioInspection } = await inspectMediabunnyInput(
        mediaInput,
        input.file,
        input.sourceId,
      );
      const { warnings } = deriveConformance(inspection, primaryVideoInspection, primaryAudioInspection);
  ...
  async open(input: MediaAdapterOpenInput): Promise<PrimaryMediaAdapterOpenResult> {
    ...
    try {
      ...
      const frameRate = primaryVideoInspection?.frameRate && primaryVideoInspection.frameRate > 0
      ...
      const kind: MediaKind = frameSource ? 'video' : audioSource ? 'audio' : primaryVideo ? 'video' : 'audio';
