  ...
  if (!track.canDecode) {
    ...
    warnings.push(
      warning(
        ...
        blocking,
        inspection.sourceId,
        `${trackLabel(track)} uses an unsupported video codec${track.codec ? ` (${track.codec})` : ''}.`,
        { codec: track.codec },
        track.trackId,
      ),
    );
  ...
  if (!track.canDecode) {
    ...
    warnings.push(
      warning(
        ...
        blocking,
        inspection.sourceId,
        `${trackLabel(track)} uses an unsupported audio codec${track.codec ? ` (${track.codec})` : ''}.`,
        { codec: track.codec },
        track.trackId,
      ),
    );
...
export function generateSourceHealthWarnings(
  ...
  const audioSampleRates = new Set(audioTracks.map((track) => track.sampleRate).filter((rate) => rate > 0));
