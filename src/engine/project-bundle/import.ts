import { deserializeProject, sourceDescriptorMismatchReasons, type ProjectDoc, type SourceDescriptor } from '../project';
...
async function loadManifestAndProject(sink: BundleDirectorySink): Promise<
  ...
  | { ok: false; reason: string; code: 'corrupt-json' | 'unsupported-bundle-schema' | 'unsupported-project-schema' }
> {
  ...
  for (const entry of manifest.sources) {
    ...
    if (entry.status !== 'embedded' || !entry.mediaAssetId) {
      report = addIntegrityItem(
        report,
        integrityItem('missing-file', 'warning', `Source ${descriptor.fileName} is not embedded in this bundle.`, {
    ...
    if (asset.fingerprint) {
      ...
      if (!fingerprintsEqual(actual, asset.fingerprint)) {
        report = addIntegrityItem(
          report,
          integrityItem('fingerprint-mismatch', 'error', `Fingerprint mismatch for ${descriptor.fileName}.`, {
            sourceId: descriptor.sourceId,
            assetId: asset.assetId,
            relativePath: asset.relativePath,
          }),
...
export async function validateProjectBundle(sink: BundleDirectorySink): Promise<BundleImportResult> {
  const loaded = await loadManifestAndProject(sink);
  if (!loaded.ok) {
    const report = addIntegrityItem(
      createEmptyIntegrityReport('unknown'),
