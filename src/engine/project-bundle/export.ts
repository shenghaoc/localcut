...
export async function exportProjectBundle(
  ...
  for (const descriptor of options.doc.sources) {
    ...
    if (!file) {
      ...
      report = addIntegrityItem(
        report,
        integrityItem('missing-file', 'warning', `Source ${descriptor.fileName} was offline at export.`, {
          sourceId: descriptor.sourceId,
        }),
      );
      continue;
    ...
    const fingerprint = await fingerprintBlob(file, {
      onProgress: (n) => options.onProgress?.({ phase: 'fingerprint', bytesDone: n, bytesTotal: file.size }),
    });
    ...
    if (!asset) {
      ...
      report = addIntegrityItem(
        report,
        integrityItem('ok', 'info', `Embedded ${file.name}.`, { assetId: asset.assetId, sourceId: descriptor.sourceId }),
      );
    } else {
