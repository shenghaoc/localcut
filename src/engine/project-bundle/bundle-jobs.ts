...
export interface BundleWorkerContext {
  ...
  postProgress: (jobId: string, phase: string, bytesDone: number, bytesTotal: number | null) => void;
...
export async function runCollectProjectMedia(
  ...
  await runExportProjectBundle(
    ctx,
    jobId,
    { mode: 'collect-media', relocate },
    outputDir,
  );
}
...
export async function runImportProjectBundle(
  ...
  try {
    if (!replaceConfirmed && (await needsReplaceConfirmation(ctx))) {
      ctx.postReplacePrompt(
        jobId,
        'Importing will replace the current project. Continue?',
      );
