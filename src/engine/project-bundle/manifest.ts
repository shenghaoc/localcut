...
function parseSourceEntry(value: unknown): BundleSourceEntry | null {
  ...
  const mediaAssetId = value.mediaAssetId === undefined ? undefined : requiredString(value.mediaAssetId) ?? undefined;
...
function parseAsset(value: unknown): BundleAsset | null {
  ...
  if (!assetId || !kind || !relativePath || byteSize === null || !originalFileName || byteSize < 0) {
    return null;
  }
  if (!Array.isArray(value.refs) || !value.refs.every((ref) => typeof ref === 'string')) return null;
  const fingerprint =
    value.fingerprint === undefined ? undefined : parseFingerprint(value.fingerprint) ?? undefined;
  const mimeType =
...
function parseManifestV1(value: Record<string, unknown>): ParseManifestResult {
  ...
  if (!bundleId || !createdAt || !appVersion || !projectId || !displayName || projectSchemaVersion === null || !policy) {
  ...
  const cacheManifest = value.cacheManifest === undefined ? undefined : parseCacheManifest(value.cacheManifest);
...
export function migrateBundle(manifest: ProjectBundleManifest, from: number, to: number): ProjectBundleManifest {
  if (from !== to) {
