...
export type CaptionAnchor = 'bottom-center' | 'bottom-left' | 'bottom-right' | 'top-center' | 'custom';
...
export const CAPTION_PRESETS: Record<
  CaptionPresetId,
  { label: string; style: Partial<TitleStyle>; anchor: CaptionAnchor; maxWidthPercent: number; lineWrap: CaptionLineWrap }
> = {
  subtitle: {
    label: 'Subtitle',
    style: {
...
export function normalizeCaptionStyle(style: Partial<CaptionStyle> | undefined): CaptionStyle {
  const presetId =
    style?.presetId === 'subtitle' || style?.presetId === 'lower-third' || style?.presetId === 'note'
      ? style.presetId
  ...
  return {
    ...
    lineWrap: style?.lineWrap === 'greedy' || style?.lineWrap === 'balanced' ? style.lineWrap : preset.lineWrap,
  };
}
...
export function sortCaptionSegments(segments: readonly CaptionSegment[]): CaptionSegment[] {
  return [...segments].sort((a, b) => (a.start === b.start ? a.id.localeCompare(b.id) : a.start - b.start));
}
