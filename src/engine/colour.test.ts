...
describe('ColorMetadata', () => {
  it('defaults to origin=none for empty hints', () => {
    const meta = colorMetadataFromHints({ primaries: null, transfer: null, matrix: null, fullRange: null });
  ...
  it('sets origin=container when metadata is present', () => {
    const meta = colorMetadataFromHints({ primaries: 'bt709', transfer: 'bt709', matrix: 'bt709', fullRange: true });
  ...
  it('detects HDR source by transfer', () => {
    const hdr = colorMetadataFromHints({ primaries: 'bt709', transfer: 'smpte2084', matrix: null, fullRange: null });
    expect(isHDRSource(hdr)).toBe(true);
  });

  it('detects HDR source by primaries', () => {
    const hdr = colorMetadataFromHints({ primaries: 'bt2020', transfer: 'bt709', matrix: null, fullRange: null });
    expect(isHDRSource(hdr)).toBe(true);
  });

  it('detects SDR as not HDR', () => {
    const sdr = colorMetadataFromHints({ primaries: 'bt709', transfer: 'bt709', matrix: 'bt709', fullRange: true });
...
describe('HDR warnings', () => {
  it('generates warnings for PQ content', () => {
    const meta = colorMetadataFromHints({ primaries: 'bt2020', transfer: 'smpte2084', matrix: null, fullRange: null });
  ...
  it('generates gamut warning for BT.2020 primaries', () => {
    const meta = colorMetadataFromHints({ primaries: 'bt2020', transfer: 'bt709', matrix: null, fullRange: null });
    const warnings = generateHDRWarnings(meta, 'clip-1');
