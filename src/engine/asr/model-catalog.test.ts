...
describe('isTrustedModelUrl', () => {
	it('allows same-origin absolute and relative URLs', () => {
		expect(isTrustedModelUrl('/models/whisper/encoder.tflite', ORIGIN)).toBe(true);
		expect(isTrustedModelUrl(`${ORIGIN}/models/whisper/encoder.tflite`, ORIGIN)).toBe(true);
	});
	...
	it('allows HTTPS URLs on allowlisted hosts (incl. HF CDN redirect targets)', () => {
		...
		expect(isTrustedModelUrl('https://storage.googleapis.com/kaggle/x.tflite', ORIGIN)).toBe(true);
	});

	it('rejects untrusted hosts and non-HTTPS schemes', () => {
		expect(isTrustedModelUrl('https://evil.example/x.tflite', ORIGIN)).toBe(false);
		expect(isTrustedModelUrl('http://huggingface.co/x.tflite', ORIGIN)).toBe(false);
		expect(isTrustedModelUrl('ftp://huggingface.co/x.tflite', ORIGIN)).toBe(false);
...
describe('catalog', () => {
	...
	it('modelById falls back to the default for unknown ids', () => {
		expect(modelById('whisper-base').id).toBe('whisper-base');
