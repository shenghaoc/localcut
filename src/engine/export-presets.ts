...
const TEMPLATE_VARIABLES = new Set(['project', 'preset', 'codec', 'date', 'time', 'range', 'index']);
...
export function mergePresetsWithBuiltIns(userPresets: readonly ExportPresetDoc[]): ExportPresetDoc[] {
...
export function expandOutputTemplate(
  template: string,
  context: OutputNameTemplateContext,
): string {
  return template.replace(TEMPLATE_VAR_RE, (_, varName: string) => {
    switch (varName) {
      case 'project': return context.project || 'Untitled';
      case 'preset': return context.preset;
      case 'codec': return context.codec;
      case 'date': return context.date;
      case 'time': return context.time;
      case 'range': return context.range;
...
export function parseExportPresetDoc(value: unknown): ExportPresetDoc | null {
  ...
  return { id, name, builtIn, codec, container, width, height, fps, videoBitrate, preset, outputTemplate };
}
