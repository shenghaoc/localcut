...
import { proxySettingsHash, sourceConformanceHash, sourceFingerprintFromDescriptor } from './cache-key';
...
export type ProxyJobPriority = 'active-range' | 'visible-filmstrip' | 'selected-source' | 'background-bin';
...
export function planProxyCandidates(
  ...
  return plans.sort((a, b) => priorityRank(a.priority) - priorityRank(b.priority) || a.sourceId.localeCompare(b.sourceId));
}
...
export function proxyStatusForAsset(
  ...
  if (asset.kind !== 'video') return { status: 'disabled', mode: 'original', reason: 'Proxy preview is for video sources.' };
