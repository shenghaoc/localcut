...
export function encodeSpillBuffer(entries: readonly RingBufferEntry[], range: SpillRange): ArrayBuffer {
	const totalSize = HEADER_SIZE + entries.reduce(
		(sum, e) => sum + ENTRY_META_SIZE + e.data.byteLength,
		0,
	);
	...
	for (const entry of entries) {
		...
		view.setUint32(offset, entry.data.byteLength, true); offset += 4;
		bytes.set(entry.data, offset); offset += entry.data.byteLength;
	}
	return buf;
}
...
export function decodeSpillBuffer(buf: ArrayBuffer): RingBufferEntry[] {
	...
	for (let i = 0; i < entryCount; i++) {
		...
		const data = new Uint8Array(buf.slice(offset, offset + byteSize)); offset += byteSize;
